...
export class AppRoot extends mixinBehaviors([AppLocalizeBehavior], PolymerElement) {
  ...
  addFocusOutline()  {
    const style = document.querySelector("body > app-root").shadowRoot.querySelector("style");
    if (style) {
  ...
  backButtonHandler(e) {
    if (this.isServerMenuOpen) {
      ...
      e.preventDefault();
      if (
        (!this.isServerListFocused && !this.isAppMenuFocused && !this.isAddServerButtonFocused) ||
        (this.isServerListFocused || this.isAppMenuFocused || this.isAddServerButtonFocused)
      ) {
  ...
  dpadHandler(e) {
    switch(e.keyCode) {
      case 13:
        ...
        if (this.isAddServerButtonFocused) {
          this.isAddServerButtonPressed = true
            const addServerView = document.querySelector("body > app-root").shadowRoot.querySelector("#addServerView");
            if (addServerView) {
              const dialog = addServerView.shadowRoot.querySelector("#addServerSheet");
              const accessKeyInput = addServerView.shadowRoot.querySelector("#accessKeyInput");
              if (dialog && accessKeyInput) {
                dialog.addEventListener("iron-overlay-opened", () => {
                  const inputElement = accessKeyInput.shadowRoot.querySelector("input");
                  if (inputElement) {
                    setTimeout(() => {
                      inputElement.focus();
                    }, 100);
            ...
            if (addServerView) {
              addServerView.addEventListener('validation-success', () => {
                const addServerButton = addServerView.shadowRoot.querySelector("#addServerButton");
                if (addServerButton) {
      ...
      case 40:
        ...
        if (!this.isServerListFocused) {
          if (this.isAddServerButtonFocused
            && !this.isAddServerButtonPressed 
            && !this.isConfirmServerButtonFocused 
            && !this.isIgnoreServerButtonFocused) {
            const serverMenu = document.querySelector("body > app-root").shadowRoot.querySelector("#serversView").shadowRoot.querySelector("server-list").shadowRoot.querySelector("server-hero-card:nth-child(1)").shadowRoot.querySelector("#serverMenuButton0");
            if (serverMenu) {
              serverMenu.focus();  // Установить фокус на кнопке
              this.isAddServerButtonFocused = false;
