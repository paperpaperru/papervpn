...
// connect.
func ConnectLocalSocksTunnel(fd int, socksPort int) (tunnel.UpdatableUDPSupportTunnel, error) {
	tun, err := tunnel.MakeTunFile(fd)
	if err != nil {
		return nil, err
	}
	t, err := newTunnel(tun, uint16(socksPort))
	if err != nil {
		return nil, err
	}
	go tunnel.ProcessInputPackets(t, tun)
	system_log.Print(fmt.Sprintf(
		"tunnel_android ConnectLocalSocksTunnel returning Tunnel for port %d", socksPort))
