...
func ConnectLocalSocksTunnel(tunWriter tunnelDarwin.TunWriter, socksPort int) (tunnel.UpdatableUDPSupportTunnel, error) {
	if tunWriter == nil {
		return nil, errors.New("must provide a TunWriter")
	}
	t, err := newTunnel(tunWriter, uint16(socksPort))
	if err != nil {
		return nil, err
	}
