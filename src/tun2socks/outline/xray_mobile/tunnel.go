...
func newTunnel(tunWriter io.WriteCloser, socksPort uint16) (tunnel.Tunnel, error) {
	if tunWriter == nil {
	...
	core.RegisterTCPConnHandler(socks.NewTCPHandler("127.0.0.1", socksPort))
	core.RegisterUDPConnHandler(socks.NewUDPHandler("127.0.0.1", socksPort, 30*time.Second))
